...
public:
    ...
    */
    void adjust_timer( util_timer* timer )
    {
        ...
        /*
            如果被调整的目标定时器处于链表尾部，或者该定时器新的超时值仍然小于其下一个定时器的超时值
            则不用调整
        */
    ...
    void del_timer( util_timer* timer )
    {
        ...
        /*
            如果链表中至少有两个定时器，且目标定时器是链表的头结点，则将链表的头节点重置为原头结点的下一个节点，
            然后删除目标定时器
        */
        if( timer == head )
        {
    ...
    */
    void tick()
    {
        ...
        util_timer* tmp = head;
        // 从头结点开始依次开始处理每个定时器，直到遇到一个尚未到期的定时器，这就是定时器的核心逻辑
        while( tmp )
        {
            ...
            // 执行完定时器中定时任务之后，就将它从链表中删除，并重置链表头节点
            head = tmp->next;
            if( head )
            {
                head->prev = NULL;
...
private:
    /*
        ...
        目标定位器timer添加到节点1st_head之后的部分链表中
    */
    void add_timer( util_timer* timer, util_timer* lst_head )
    {
        ...
        /*
            遍历1st_head节点之后的部分链表，直到找到一个超时时间大于目标定时器的超时时间的节点，并将目标定时器插入该节点之前
        */
        while( tmp )
        {
        ...
        /*
            如果遍历完1st_head节点之后的部分链表，仍未找到超时时间大于目标定时器的超时时间的节点，则将目标定时器插入链表尾部，
            并把它设置为链表新的尾节点
        */
        if( !tmp )
        {
