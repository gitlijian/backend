...
public:
    ...
    int time_slot;  // 记录定时器属于时间轮的哪个槽（对应的链表，下同）
...
public:
    ...
    tw_timer* add_timer( int timeout )
    {
        ...
        int ticks = 0;
        /*
            下面根据待插入定时器的超时值计算它将在时间轮转动多少个滴答后被触发，并将该滴答数
            存储于变量ticks中。如果待插入定时器的超时值小于时间轮的槽间隔SI，则将ticks向上折合为1，
